...
def llm_answer(query: str, top_k: Optional[int] = None) -> str:
    ...
    Args:
        query: User's question about recipes
        top_k: Number of recipes to use as context (optional)
...
def llm_answer_stream(query: str, top_k: Optional[int] = None) -> Generator[str, None, None]:
    """
    ...
    Yields:
        Individual chunks of the response

    Example:
        >>> for chunk in llm_answer_stream("How to cook chocolate cookies?"):
        ...     print(chunk, end="", flush=True)
    """
    service = _get_service()
    yield from service.query_stream(query, top_k)
